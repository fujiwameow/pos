...
class Pos_Table_Api extends API_Base {
	...
	public function update_table() {
		...
		$is_updated = false;
		file_put_contents(WP_CONTENT_DIR."/class-pos-table-payload-api.txt",print_r($this->payload,true),FILE_APPEND);
		if ( $old_object ) {
			$table_obj = new Mapbd_Pos_Table();
			if ( $table_obj->set_from_array( $this->payload, false ) ) {
				...
				if ( $is_updated ) {
					...
					$this->response->set_response( false, appsbd_get_msg_api(), $table_obj );
				}
				return $this->response->get_response();
			}
		}
