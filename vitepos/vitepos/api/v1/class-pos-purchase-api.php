...
class Pos_Purchase_Api extends API_Base {
	...
	public function update_wc_stock( &$product, $purchase, $add_to_list, $outlet_id = '' ) {
		$outlet_stock = floatval( $purchase->stock_quantity );
		$stock        = floatval( $product->get_stock_quantity() ) + $outlet_stock;
		if ( ! POS_Settings::is_stockable() ) {
			...
			if ( $product->meta_exists( '_vt_prev_purchase_price' ) ) {
				...
				if ( ! empty( $outlet_id ) && POS_Settings::is_stockable() ) {
		...
		if ( $product->save() ) {
			/**
			 * Its for update stock
			 *
			 * @since 1.4
			 */
			do_action( 'apbd-vtpos/action/stock-update', $product, $stock, $outlet_id );
			return true;
		} else {
						...
						if ( $transfer_obj->save() ) {
							foreach ( $transfer_items as $transfer_item ) {
								...
								if ( ! $transfer_item->save() ) {
									 ...
									 * @since 1.0.0
									 */
									$std->stock = apply_filters( 'vitepos/filter/outlet-stock', 0, $std->product_id, $transfer_obj->transfer_from );
									$data[]     = $std;
						...
						if ( $purchase_obj->save() ) {
							foreach ( $purchase_items as $purchase_item_obj ) {
								...
								if ( $purchase_item_obj->Save() ) {
									 ...
									 * @since 1.0.0
									 */
									$std->stock = apply_filters( 'vitepos/filter/outlet-stock', 0, $std->product_id, $purchase_obj->warehouse_id );
									$data[]     = $std;
								} else {
									$is_item_save_ok = false;
								}
							...
							if ( $is_item_save_ok ) {
								if ( ! empty( $data ) ) {
									 ...
									 * @since 2.0
									 */
									do_action( 'vitepos/action/stock-updated', $data ,$purchase_obj->warehouse_id);
								}
	...
	public function stock_logs( $data ) {
		if ( ! empty( $data['id'] ) ) {
			...
			$product->logs         = $log_obj->select_all();
