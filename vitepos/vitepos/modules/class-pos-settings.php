...
class POS_Settings extends BaseModule {
	...
	public static function get_settings( $type = '' ) {
		...
		$basic_settings['stockable'] = self::get_module_instance()->get_stock_settings()->is_stockable;
		if ( ! empty( $basic_settings['is_kitchen'] ) && 'Y' == $basic_settings['is_kitchen'] ) {
			$obj        = new \stdClass();
			$obj->title = 'Completed';
			$obj->val   = 'completed';
		...
		$basic_settings['fps'] = hash( 'crc32b', $basic_settings['stockable'] . $basic_settings['barcode_field'] . $basic_settings['pos_mode'] . json_encode( $product_status ) );
	...
	public function is_stock_linked_with() {
		...
		if ( ( ! empty( $stock_settings->is_linked_outlet ) ) && ( 'Y' == $stock_settings->is_linked_outlet ) && ( ! empty( $stock_settings->linked_outlet ) ) ) {
			return true;
		}
